...
def filmes_homepage(name,url):
    ...
    if html_source:
        match = re.compile('<img .+? data-src="(.+?)" alt=".+?" style=".+?"/><span>(.+?)</span><a href="(.+?)"').findall(html_source_trunk[0])
        totalit = len(match)
...
def menu_pedidos(url):
    ...
    pag_total = match[-1]
    try: addDir_reg_menu("[COLOR green]Página " + str(pag_actual) +"/"+str(pag_total) + " |[B] Seguinte >>[/COLOR][/B]",'http://ratotv.xyz/requests/page/' + str(int(pag_actual)+1) + '/',33,artfolder+'seta.jpg',True,fanart=fanart_rato_tv)
    except: pass
    pedidos_view()
...
def pedir_id(url):
    ...
    source = post_page_free("http://ratotv.xyz/engine/ajax/mws-film.ajax.php",mydata);print source
...
def mensagens_conta():
    try: html_source = post_page("http://ratotv.xyz/index.php?do=pm&folder=inbox",selfAddon.getSetting('login_name'),selfAddon.getSetting('login_password'))
    except: html_source = ""
...
def apagar_pm(url):
    ...
    try: html_source = post_page("http://ratotv.xyz/index.php?do=pm&doaction=readpm&pmid="+pm_id,selfAddon.getSetting('login_name'),selfAddon.getSetting('login_password'))
    except: html_source
    if html_source:
...
def play_from_outside(name,url):
    ...
    match = re.compile('<img src="/templates/ratotvv2/dleimages/comments-img2.png"/><a href="http://ratotv.xyz/(.+?)/">.+?</a>').findall(html_source)
    if match[0] == 'tvshows':
        proceed = False
